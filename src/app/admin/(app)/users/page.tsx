  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--color-muted)] mt-1">Manage users and apply restrictions.</p>
      </div>
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 grid gap-3">
          {users.map((u) => (
            <div key={u.id} className="border border-[var(--color-border)] rounded-lg p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="font-medium truncate">{u.name}</div>
                  <div className="text-sm text-[var(--color-muted)] mt-1 truncate">{u.email} • {u.role}</div>
                </div>
          ...
          {!loading && users.length === 0 ? (
            <div className="text-sm text-[var(--color-muted)]">No users found.</div>
          ) : null}
        </div>
      </Card>
    </div>
